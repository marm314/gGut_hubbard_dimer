...
\bottomrule
\end{tabular}
\caption{Solutions of the direct minimization at $U/t=2$, $\Ng=2$. Backward: started from the
$(R,\lambda)$ seed of \texttt{examples/U2\_backward}. Forward: converged from a random start (the
seed of \texttt{examples/U2\_forward} leads to the backward solution, see below). The energies agree with
\texttt{gga\_dimer.py}.}
\label{tab:ni}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{The forward seed does not stay on the forward minimum.} A single SLSQP pass
    started from the forward seed stops early ($E=-3.12499887$, projected gradient
    $4\times10^{-3}$, descent directions with $\Delta E\simeq-8\times10^{-5}$ exist). A second
    pass continues downhill, \emph{not} to the forward minimum $-3.1249999925$ but to the lower
    ghost-assisted (backward) solution $-3.1256614043$, and further passes stay there. The forward
    minimum listed in Table~\ref{tab:ni} was reached from a random start; it is a local minimum with
    a small basin, and the seed lies outside it. Consequently the ``forward'' U-scan of
    \texttt{scan\_not\_imp\_U.py}, seeded from that point, converges to the ghost-assisted branch
    at $U/t=2$ (it stays on the plain-Gutzwiller branch only for $U/t\lesssim0.8$).
\end{itemize}
...
python3 not_imp_gga_dimer.py --U <U/t> --Ng <even>   [--t 1.0] [--nstarts 10] [--seed 0]
                             ...
                             [--omega <w_max> --eta <eta> --domega <step>]   # spectral function
...
\paragraph{Spectral function.} If \texttt{-{}-omega}, \texttt{-{}-eta} and \texttt{-{}-domega} are all
given (any subset is an error), the spectral function of the solution is computed exactly as in the
post-processing section of \texttt{code.tex}: $G(\omega)=[(\omega+i\eta)\mathbf I-\Hqp]^{-1}$ with the
$\Hqp$ of the reconstructed $(R,\lambda)$ (Section~\ref{sec:reconstruction}), then
$G_{\mathrm{phys}}=R_{\mathrm{mat}}^{T}G\,R_{\mathrm{mat}}$ with $R_{\mathrm{mat}}=\mathrm{diag}(R^0,R^1)$ and
$A(\omega)=-\tfrac2\pi\,\mathrm{Im\,Tr}\,G_{\mathrm{phys}}(\omega)$ on the grid $-\omega_{\max},\dots,\omega_{\max}$
in steps of $d\omega$, written to \texttt{A\_omega.txt}; the routine is the one of \texttt{gga\_dimer.py}
(\texttt{compute\_spectral\_function}), and the sum rule $\int A\,d\omega\to4$ is printed.
The only input from the minimization is $(R,\lambda)$, so $A(\omega)$ inherits the ambiguity of the
nearly free direction of $\lambda$ described in Section~\ref{sec:reconstruction}; at $U/t=10$ (the
\texttt{examples/U10} seed, $\omega_{\max}=15$, $\eta=d\omega=0.05$) it gives $\int A\,d\omega=3.9615$ and agrees with the
$A(\omega)$ of \texttt{gga\_dimer.py} for the same case to $6\times10^{-5}$ (maximum of $A$: $6.48$).
...
least-squares sense (\texttt{numpy.linalg.lstsq}, \texttt{rcond}$=10^{-6}$, minimum-norm
...
\begin{itemize}
  ...
  \item \emph{Nearly free direction.} For $U/t\gtrsim7$ ($\Ng=2$) the smallest singular value of
    the system is $\sim10^{-7}$, while the next one is $\sim0.15$: one direction of $\lambda$
    is almost undetermined by the stationarity conditions (it belongs to the nearly decoupled
    bath orbital, whose quasiparticle level is barely constrained). With a cutoff as small as
    $10^{-10}$ that direction is fixed by rounding noise, the residual is unchanged, and the two
    middle quasiparticle levels jump erratically with $U$ (values between $-3$ and $+4$ were
    seen). The cutoff $\texttt{rcond}=10^{-6}$ (relative to the largest singular value) removes
    it, i.e.\ takes the minimum-norm $\lambda$; results are identical for $10^{-6}$ and
    $10^{-4}$, and the two levels then vary smoothly and stay at $\approx10^{-3}$ (at the Fermi
    level, as expected for a decoupled bath) with the residual $\lesssim8\times10^{-7}$. The
    position of those two levels is therefore a \emph{choice} of the minimum-norm solution
    rather than something determined by $(x,X)$ alone.
